...
export function Navbar({ className, items = [], cta, ...props }: NavbarProps) {
  ...
  return (
    <header
      ...
      {...props}
    >
      <div className="pointer-events-auto flex w-full max-w-5xl items-center gap-4 rounded-pv-lg border border-[var(--pv-border)] bg-[var(--pv-surface)]/75 px-4 py-3 shadow-[0_28px_60px_-30px_rgba(32,32,72,0.55)] backdrop-blur-xl transition-colors duration-300 dark:bg-[var(--pv-bg)]/70 md:gap-6 md:px-6">
        ...
        <div className="ml-auto flex items-center gap-2 md:gap-3">
          ...
          {items.length > 0 && (
            <Sheet>
              ...
              <SheetContent side="right" className="w-full max-w-xs border-[var(--pv-border)] bg-[var(--pv-bg)] text-left dark:bg-[var(--pv-surface)]">
                <SheetHeader className="flex flex-row items-center justify-between gap-3">
                  <Link href="/" className="flex items-center gap-2 text-lg font-semibold" aria-label="PixelVerse Studios">
                      ...
                      <SheetClose asChild key={item.href}>
                        <Link
                          ...
                          className={cn(
                            ...
                            'text-[var(--pv-text)] hover:bg-[var(--pv-surface)] hover:text-[var(--pv-text)] dark:hover:bg-[var(--pv-bg)]/80',
                            isActive &&
