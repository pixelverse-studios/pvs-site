...
export function CampaignDetailModal({ campaign, isOpen, onClose }: CampaignDetailModalProps) {
  const opened = isOpen && campaign !== null;
  const [display, clearDisplay] = useClosingOverlayValue(campaign, opened);
  return (
    <Modal
      opened={opened}
      onClose={onClose}
      onExitTransitionEnd={clearDisplay}
      title={display ? `Campaign details: ${display.subject}` : 'Campaign details'}
    ...
    >
      {display && <CampaignDetailContent campaign={display} isOpen={opened} />}
    </Modal>
  );
}

function CampaignDetailContent({ campaign, isOpen }: { campaign: Campaign; isOpen: boolean }) {
                ...
                <tbody>
                  {sortedRecipients.map((recipient, idx) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-4 py-2.5 font-medium" style={{ color: 'var(--pv-text)' }}>
